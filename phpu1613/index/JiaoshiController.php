<?php
session_start();
class JiaoshiController extends CommonController {
    ...
    public function login(){
        ...
        $password = isset($_REQUEST['password'])?$_REQUEST['password']:"";
                        $sql = "select * from `jiaoshi` where `jiaoshibianhao` = '".$username."' and `mima` = '".$password."'";
    ...
    public function register(){
        ...
        $postData = json_decode($tmpData,true);
                        ...
                        $colum = "jiaoshibianhao";
        $trues = "select * from `jiaoshi` where `jiaoshibianhao` = '".$postData[$colum]."'";
                                                                                                                                		        $result = table_sql($trues);
        if($result->num_rows<1){
			$keyArr = $valArr = array();
			foreach ($postData as $key => $value){
                if (in_array($key, $this->columData) && !empty($value)){
			...
			$key = implode(',',$keyArr);
			$v = implode(',',$valArr);
                        $sql = "INSERT INTO `jiaoshi` (`id`,".$key.") VALUES (".time().",".$v.")";
                        $result = table_sql($sql);
    ...
    public function resetPass(){
        $username = $_REQUEST['username'];
                        $counts = "select * from `jiaoshi` where jiaoshibianhao = '".$username."'";
        $cotte = table_sql($counts);
        if($cotte->num_rows<1){
            exit(json_encode(['code'=>500,'msg'=>"用户名错误。"]));
        }
        $sql = "update jiaoshi set mima = '123456' where jiaoshibianhao = '".$username."'";
                                                                                                                                        		        $result = table_sql($sql);
    ...
    public function page(){
        ...
        if ($base['isAdmin']!=1 || ($base['isAdmin']==1 && $base['tablename'] != "users")){
            ...
            if (isset($_SESSION[$md5]) && in_array($colum, $columData)){
                if($base['tablename'] == '${column.authTable}'){
                    $where .= " and `jiaoshibianhao` = '".$_SESSION[$md5]."'";
                }
            }
        }
                                                                                                                                        		        
                                        $sql = "select * from `jiaoshi` ".$where;
        $count = table_sql($sql);
        if ($count->num_rows < 1){
            $numberCount = 0;
        }else{
        ...
        $startCount = ($page-1)*$limt;
                    $lists = "select * from `jiaoshi` ".$where." order by ".$sort." ".$order." limit ".$startCount.",".$limt;
    ...
    public function save(){
        ...
        if (!empty($tmpData)&& isset($tmpData)){
            $postData = json_decode($tmpData,true);
                ...
                $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                        $sql = "INSERT INTO `jiaoshi` (`id`,".$k.") VALUES (".time().",".$v.")";
                $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function add(){
        ...
        $k = implode(',',$keyArr);
        $v = implode(',',$valArr);
                $sql = "INSERT INTO `jiaoshi` (".$k.") VALUES (".$v.")";
        $result = table_sql($sql);
		        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function update(){
        ...
        $value = implode(',',$v);
         $sql = "UPDATE jiaoshi SET ".$value." where id = ".$id;
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function delete(){
        ...
        $sql = "delete from jiaoshi WHERE id in({$str})";
        $result = table_sql($sql);
        exit(json_encode(['code'=>0]));
    }
    /**
    ...
    public function info($id=false){
        ...
        if (!empty($id)){
            ...
            $where = "`name` = ".$name;
                        ...
                        $sql = "select * from `jiaoshi` where ".$where;
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
            while($row = $result->fetch_assoc()) {
    ...
    public function value(){
        ...
        $sql = "SELECT ".$xColumnName.",sum(".$yColumnName.") total FROM jiaoshi group by ".$xColumnName;
        if(urldecode($request[6]) == '日') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m-%d') ".$xColumnName.", sum(".$yColumnName.") total FROM jiaoshi GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m-%d')";
        }
        if(urldecode($request[6]) == '月') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y-%m') ".$xColumnName.", sum(".$yColumnName.") total FROM jiaoshi GROUP BY DATE_FORMAT(".$xColumnName.", '%Y-%m')";
        }
        if(urldecode($request[6]) == '年') {
            $sql = "SELECT DATE_FORMAT(".$xColumnName.", '%Y') ".$xColumnName.", sum(".$yColumnName.") total FROM jiaoshi GROUP BY DATE_FORMAT(".$xColumnName.", '%Y')";
        }
        $result = table_sql($sql);
        if ($result->num_rows > 0) {
            // 输出数据
    ...
    public function remind($columnName,$type){
        ...
        $sql = "select * from `jiaoshi` where 1 ".$where;
        $result = table_sql($sql);
