<template>
    ...
    <view class="cu-bar bg-white search" :style="[{top:CustomBar + 'px'}]">
			<picker v-if="queryList.length>1" mode="selector" :range="queryList" range-key="queryName" :value="queryIndex" @change="queryChange" style="padding-left: 20upx;">
				...
				<input v-model="searchForm.jiaofeibianhao" type="text" placeholder="缴费编号" ></input>
			</view>
                  			<view v-if="queryIndex==1" class="search-form round">
				<text class="cuIcon-search"></text>
				<input v-model="searchForm.jiaofeileixing" type="text" placeholder="缴费类型" ></input>
			</view>
        ...
        <view @tap="onDetailTap(product)" v-if="index%6==0" class="list-item" :style='{"padding":"20rpx 30rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"14rpx 0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255, 255, 255, 1)","borderRadius":"60rpx 0 60rpx 0","borderWidth":"4rpx","width":"100%","borderStyle":"solid","height":"auto"}' style="display: flex;">
                                                                                                                                  <image :style='{"padding":"0","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"rgba(22, 113, 193, 1)","backgroundColor":"rgba(255,0,0,0)","borderRadius":"0","borderWidth":"4rpx","width":"240rpx","borderStyle":"solid","height":"180rpx"}' mode="aspectFill" :src="product.feiyongtupian?baseUrl+product.feiyongtupian.split(',')[0]:''"></image>
                                                                                                                                  <view class="list-item-body" style="flex: 1;">
            <view :style='{"padding":"20rpx","boxShadow":"0 0 12rpx rgba(255,0,0,0)","margin":"0","borderColor":"red","backgroundColor":"rgba(255,0,0,0)","color":"#333","textAlign":"left","borderRadius":"0","borderWidth":"0","width":"100%","lineHeight":"40rpx","fontSize":"28rpx","borderStyle":"solid"}' class="title">{{product.jiaofeibianhao}}</view>
                                                                                                                                                                                                                                                                      </view>
                                                                                                                                                                                                                  <view style="display: flex;justify-content: space-between;">
            <text v-if="userid && isAuth('jiaofeixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="!userid && isAuthFront('jiaofeixinxi','修改')" class="cuIcon-edit" @click.stop="onUpdateTap(product.id)">修改</text>
            <text v-if="userid && isAuth('jiaofeixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
            <text v-if="!userid && isAuthFront('jiaofeixinxi','删除')" class="cuIcon-delete" @click.stop="onDeleteTap(product.id)">删除</text>
          </view>
        </view>
	...
	export default {
		...
		methods: {
			...
			queryChange(e) {
				...
				this.searchForm.jiaofeibianhao="";
				this.searchForm.jiaofeileixing="";
			},
			//类别搜索
			...
			async upCallback(mescroll) {
				...
				if(this.searchForm.jiaofeibianhao){
					params['jiaofeibianhao'] = '%' + this.searchForm.jiaofeibianhao + '%'
				}
				if(this.searchForm.jiaofeileixing){
                ...
                let res = {}
                if(this.userid) {
                    res = await this.$api.page(`jiaofeixinxi`, params);
                } else {
                    res = await this.$api.list(`jiaofeixinxi`, params);
                }
				// 如果是第一页数据置空
			...
			onDeleteTap(id){
				...
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							await _this.$api.del('jiaofeixinxi', JSON.stringify([id]));
							_this.hasNext = true
							// 重置分页参数页数为1
			...
			async search(){
				...
				if(this.searchForm.jiaofeibianhao){
					searchForm['jiaofeibianhao'] = '%' + this.searchForm.jiaofeibianhao + '%'
				}
				if(this.searchForm.jiaofeileixing){
					searchForm['jiaofeileixing'] = '%' + this.searchForm.jiaofeileixing + '%'
				}
                let res = {};
                if(this.userid) {
                    res = await this.$api.page(`jiaofeixinxi`, searchForm);
                } else {
                    res = await this.$api.list(`jiaofeixinxi`, searchForm);
                }
				// 如果是第一页数据置空
